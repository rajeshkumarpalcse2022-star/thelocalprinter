...
const UserChat = () => {
  ...
  const handleStartChat = useCallback(async () => {
    ...
    try {
      const res = await createConversationByPublicId(newChatId.trim());
      ...
      const freshRes = await getConversations("VENDOR");
      const freshConv = freshRes.data.data.conversations.find(
        (c) => c._id === conv._id
      );
      if (freshConv) selectConversation(freshConv);
      else {
        setActiveConversation(conv);
        setMessages([]);
        setLoadingMessages(true);
        const msgRes = await getMessages(conv._id);
  ...
  }, [newChatId, fetchConversations, selectConversation, scrollToBottom, joinConversation]);
    ...
    <div className="flex h-[calc(100vh-4rem)] flex-col">
      <div className="flex items-center justify-between rounded-t-lg border border-b-0 bg-white px-4 py-3 dark:bg-gray-950">
        <div className="flex items-center gap-3 min-w-0">
          <h2 className="text-lg font-semibold shrink-0">Chat</h2>
          {user?.publicId && <div className="min-w-0 hidden sm:block"><CopyableId id={user.publicId} /></div>}
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-2">
              <Label className="text-sm">Vendor Public ID</Label>
              <Input
                placeholder="Enter Vendor ID (e.g. VND-8F42K7)"
                value={newChatId}
                onChange={(e) => {
                  setNewChatId(e.target.value.toUpperCase());
                  setNewChatError("");
                }}
                onKeyDown={(e) => {
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={!newChatId.trim() || creatingChat}
              className="bg-blue-500 hover:bg-blue-600"
            >
